 ...
 * test_heap.c
 *
 * This file implements tests of the simulated memory management
 * functions declared in mm_heap.c. This tests read data files
 * that specify calls to mm_malloc(), mm_realloc(), and mm_free()
 * and report unexpected conditions such as leaked or corrupted
 * memory.
 *
 * @since 2019-02-20
 * @author philip gust
...
int main(int argc, char *argv[]) {
	...
	bool verbose = false;
	bool debug = false;
	fixup(argc, argv);  // works around Eclipse debugging error
		...
		while (fscanf(tracefile, "%s", type) != EOF) {
			...
			case 'f':
				...
				if (blocks[index] == NULL) {
					...
					for (int i = 0; i < block_sizes[index]; i++) {
						if (*((char*)blocks[index]+i) != (char)(index & 0xFF)) {
							if (debug) fprintf(stderr, "  Block %u has unexpected data before free.\n", index);
							nerrors++;
							break;
						}
					}
